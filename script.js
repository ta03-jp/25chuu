...
function selectClothing(type) {
  ...
  // 全てのボックスから選択状態を解除
  document.querySelectorAll(".clothing-box").forEach((box) => {
...
function goToNext() {
  switch (selectedType) {
    case "tops":
      window.location.href = "CaptureTops.html";
      break;
    case "bottoms":
      window.location.href = "CaptureBottoms.html";
      break;
    case "onepiece":
      window.location.href = "CaptureOnepiece.html";
      break;
    default:
      alert("服の種類を選択してください");
  }
...
function sendImageDataToAPI() {
  if (templateName === "シャツ・ブラウス") {
    fieldName = "longshirt[]";
  } else if (templateName === "オーバーサイズTシャツ") {
    fieldName = "bigTshirt[]";
  } else {
    fieldName = "shortT[]";
  }
  ...
  fetch("https://smashing-kiwi-visually.ngrok-free.app/process_images", {
    method: "POST",
    body: formData,
  })
    .then((response) => {
